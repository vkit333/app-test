import { symbol, debugToString, Stack, assign, initializeGuid, fillNulls, _WeakSet, dict, EMPTY_ARRAY, decodeHandle, isNonPrimitiveHandle, unreachable, unwrapTemplate, isErrHandle, unwrapHandle, clearElement, isSmallInt, encodeImmediate, encodeHandle, constants, decodeImmediate, isHandle } from '@glimmer/util';
...
import { UNDEFINED_REFERENCE, valueForRef, createComputeRef, childRefFor, TRUE_REFERENCE, FALSE_REFERENCE, createDebugAliasRef, isConstRef, createPrimitiveRef, NULL_REFERENCE, createIteratorRef, updateRef, createIteratorItemRef } from '@glimmer/reference';
import { $pc, $sp, $ra, $fp, $v0, $t0, $t1, $s0, $s1, isLowLevelRegister } from '@glimmer/vm';
import { CONSTANT_TAG, valueForTag, validateTag, INITIAL, beginTrackFrame, endTrackFrame, consumeTag, track, updateTag, resetTracking, runInTrackingTransaction } from '@glimmer/validator';
...
, (vm, {
  ...
  let owner = vm[CONSTANTS].getValue(decodeHandle(_owner));
...
, (vm, {
  ...
  let owner = vm[CONSTANTS].getValue(_owner);
...
, (vm, {
  ...
  if (vm.env.debugRenderTree !== undefined) {
    ...
    if (vm.stack.peek() === vm[ARGS]) {
      ...
      let names = vm[CONSTANTS].getValue(_names);
      vm[ARGS].setup(vm.stack, names, [], 0, true);
      args = vm[ARGS].capture();
    }

    let template = hasStaticLayout(instance.capabilities, manager) ? manager.getStaticLayout(definition.state) : manager.getDynamicLayout(state, vm.runtime.resolver); // For tearing down the debugRenderTree

    vm.associateDestroyable(instance);

    if (hasCustomDebugRenderTreeLifecycle(manager)) {
      let nodes = manager.getDebugCustomRenderTree(instance.definition.state, instance.state, args, template);
      nodes.forEach(node => {
        let {
          bucket
        } = node;
...
, (vm, {
  ...
  let owner = constants$$1.getValue(decodeHandle(_owner));
  let outerSymbols = constants$$1.getArray(_symbols);
  let evalInfo = constants$$1.getValue(decodeHandle(_evalInfo));
  let handle = vm.runtime.resolver.lookupPartial(name, owner);
  let definition = vm.runtime.resolver.resolve(handle);
  let {
    symbolTable,
    handle: vmHandle
...
function getModifierManager(owner, definition) {
  ...
  if (factory !== undefined) {
    ...
    if (DEBUG && !isInternalModifierManager(manager) && !FROM_CAPABILITIES.has(manager.capabilities)) {
...
function getComponentManager(owner, definition) {
  ...
  if (factory !== undefined) {
    ...
    if (DEBUG && !isInternalComponentManager(manager) && !FROM_CAPABILITIES.has(manager.capabilities)) {
      // TODO: This error message should make sense in both Ember and Glimmer https://github.com/glimmerjs/glimmer-vm/issues/1200
...
export { clear, ConcreteBounds, CursorImpl, capabilityFlagsFrom, hasCapability, resetDebuggerCallback, setDebuggerCallback, CurriedComponentDefinition, curry, isCurriedComponentDefinition, DEFAULT_CAPABILITIES, MINIMAL_CAPABILITIES, helper$1 as DOMChanges, DOMChangesImpl as IDOMChanges, DOMTreeConstruction, isWhitespace, normalizeProperty, DynamicScopeImpl, PartialScopeImpl, runtimeContext, EnvironmentImpl, inTransaction, getDynamicVar, renderComponent, renderMain, renderSync, UpdatingVM, VM as LowLevelVM, EMPTY_ARGS, EMPTY_NAMED, EMPTY_POSITIONAL, createCapturedArgs, reifyArgs, reifyNamed, reifyPositional, DynamicAttribute, dynamicAttribute, SimpleDynamicAttribute, clientBuilder, NewElementBuilder, UpdatableBlockImpl, RemoteLiveBlock, isSerializationFirstNode, RehydrateBuilder, rehydrationBuilder, SERIALIZATION_FIRST_NODE_STRING, destroy, registerDestructor, unregisterDestructor, associateDestroyableChild, isDestroying, isDestroyed, enableDestroyableTracking, assertDestroyablesDestroyed, destroyChildren as _destroyChildren, getComponentTemplate, setComponentTemplate, templateOnlyComponent, isTemplateOnlyComponent, TemplateOnlyComponent, getOwner, setOwner, OWNER, getComponentManager, setComponentManager, getHelperManager, setHelperManager, getModifierManager, setModifierManager, buildCapabilities, BaseInternalComponentManager, BaseInternalModifierManager, isInternalComponentManager, isInternalModifierManager, isInternalHelper, SimpleComponentManager, TEMPLATE_ONLY_COMPONENT };