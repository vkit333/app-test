...
import { enumerableSymbol, symbol, HAS_NATIVE_PROXY, getDebugName, guidFor, isProxy, HAS_NATIVE_SYMBOL, isEmberArray, isObject, uuid } from '@ember/-internals/utils';
...
import { isUpdatableRef, updateRef, valueForRef, createComputeRef, UNDEFINED_REFERENCE, createConstRef, childRefFor, childRefFromParts, createPrimitiveRef, createUnboundRef, isInvokableRef, isConstRef, NULL_REFERENCE, createInvokableRef, createReadOnlyRef, createDebugAliasRef } from '@glimmer/reference';
import { normalizeProperty, associateDestroyableChild, buildCapabilities, EMPTY_ARGS, EMPTY_NAMED, EMPTY_POSITIONAL, getHelperManager, isDestroyed, isDestroying, isInternalHelper, setHelperManager, BaseInternalComponentManager, registerDestructor, reifyPositional, reifyNamed, reifyArgs, createCapturedArgs, curry, getComponentManager, getComponentTemplate, getDynamicVar, getModifierManager, isInternalComponentManager, isInternalModifierManager, clientBuilder, destroy, DOMChanges, DOMTreeConstruction, inTransaction, renderMain, runtimeContext, setComponentManager, rehydrationBuilder } from '@glimmer/runtime';
export { setComponentTemplate, getComponentTemplate, DOMChanges, DOMTreeConstruction, isSerializationFirstNode } from '@glimmer/runtime';
...
import { _WeakSet, unwrapTemplate, EMPTY_ARRAY, dict, unwrapHandle } from '@glimmer/util';
...
class OutletComponentManager extends BaseInternalComponentManager {
...
class CurlyComponentManager extends BaseInternalComponentManager {
...
class CustomComponentManager extends BaseInternalComponentManager {
...
class InternalManager extends BaseInternalComponentManager {
...
function assertCallbackIsFn(callbackRef) {
  assert(`You must pass a function as the \`fn\` helpers first argument, you passed ${callbackRef ? valueForRef(callbackRef) : callbackRef}. While rendering:\n\n${callbackRef === null || callbackRef === void 0 ? void 0 : callbackRef.debugLabel}`, callbackRef && (isInvokableRef(callbackRef) || typeof valueForRef(callbackRef) === 'function'));
}
...
class MountManager extends BaseInternalComponentManager {
...
class RuntimeResolverImpl {
  ...
  lookupHelper(name, owner) {
    ...
    let helper$$1 = this._lookupHelper(name, owner);
  ...
  _lookupHelper(_name, owner) {
    assert(`You attempted to overwrite the built-in helper "${_name}" which is not allowed. Please rename the helper.`, !(this.builtInHelpers[_name] && owner.hasRegistration(`helper:${_name}`)));
    ...
    let name = _name;
    const factory = owner.factoryFor(`helper:${name}`) || owner.factoryFor(`helper:${name}`);
    ...
    assert(`internal helpers are not supported via \`getHelperManager\` yet, found an internal manager for ${name}`, !isInternalHelper(manager)); // For classic class based helpers, we need to pass the factoryFor result itself rather
  ...
  _lookupModifier(name, owner) {
    ...
    if (builtin === undefined) {
      ...
      if (modifier !== undefined) {
        ...
        assert(`Internal modifier managers are not supported via \`getModifierManager\` yet, found an internal manager for ${name}`, !isInternalModifierManager(manager));
...
function refineBlockSyntax(name, params, hash, blocks, context) {
  ...
  assert(`A component or helper named "${name}" could not be found`, context.meta.owner.hasRegistration(`helper:${name}`));
  assert(`Helpers may not be used in the block form, for example {{#${name}}}{{/${name}}}. Please use a component, or alternatively use the helper in combination with a built-in Ember helper, for example {{#if (${name})}}{{/if}}.`, !(() => {
    ...
    const owner = context.meta.owner;
...
var ComponentTemplate = templateFactory({
  "id": "q4vmP/CB",
...
export { RootTemplate, Checkbox, TextField, TextArea, LinkComponent, Component, Helper, helper, SafeString, escapeExpression, htmlSafe, isHTMLSafe, Renderer, InertRenderer, InteractiveRenderer, _resetRenderers, renderSettled, getTemplate, setTemplate, hasTemplate, getTemplates, setTemplates, setupEngineRegistry, setupApplicationRegistry, registerMacros as _registerMacros, experimentalMacros as _experimentalMacros, INVOKE, OutletView, setComponentManager$1 as setComponentManager, capabilities, capabilities$1 as modifierCapabilities, helperCapabilities, invokeHelper };
